...
## Running the testing subgraph

This repository now contains a second, independent subgraph under `testing/`.
It runs on port `4002` and provides deterministic fixture data through
`testProducts` and `testProduct`. It also extends the federated `Product` entity
with `testStatus` and `testScore` fields.
...
The products subgraph is available at `http://localhost:4001`, the testing
subgraph at `http://localhost:4002`, and the composed supergraph through Router
at `http://localhost:4000`.

Through Router, this query combines fields from both subgraphs:
...
See `docs/federation-architecture.md` for the request flow and
`docs/federation-architecture.excalidraw` for the architecture diagram.

For the internal query-plan flow and a production platform blueprint, see
`docs/federation-internals.md` and `docs/production-federation-platform.md`.
...
The local `rover dev` command composes the schemas from `supergraph.yaml`; it
does not publish or register those schemas in GraphOS Studio. To see `testing`
in Studio, publish it to the same GraphOS graph variant used by your Router and
give it a reachable routing URL. Studio cannot discover a service running on
`localhost`.
